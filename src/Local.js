...
export default function Local () {
  ...
  console.log("ref", textContentRef)
    if(textContentRef.current){
      console.log("Local Textcontent TEST", textContentRef.current.value)
      setSelectedPlace(textContentRef.current.textContent)
    }
  }
...
{/* { 
    ...
    || <div className="empty-map">검색된 결과가 없습니다. 우리 '-구' 검색해 보세요.</div>
      ...
      <div className="local">
        <button onClick={(e)=> handleClickLocal(e)}>종로구</button>
        <button onClick={(e)=> handleClickLocal(e)}>중구</button>
        <button onClick={(e)=> handleClickLocal(e)}>용산구</button>
        <button onClick={(e)=> handleClickLocal(e)}>성동구</button>
        <button onClick={(e)=> handleClickLocal(e)}>광진구</button>
      </div>
