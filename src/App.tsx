        ...
        <div className="search-wrap">
          ...
          <form onSubmit={handleResult}>
            <div className="input-wrap">
              <input type="text" value={searchKeyword} onChange={handleChangeInput} placeholder="'-구'를 입력해주세요! 예시)종로구"></input>
            </div>
          </form>
        ...
        newLibrary.map((item) => { 
          return(
            <BookList 
              key={item.doc.no}
              no={item.doc.no}
              img={item.doc.bookImageURL}
              name={item.doc.bookname} 
              author={item.doc.authors} 
              genres={item.doc.class_nm} 
